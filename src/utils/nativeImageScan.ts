...
// (ZXing, see ScannerScreen.decodeImageFromFile) built on canvas/ImageData,
// which doesn't exist in React Native — this is the native-only equivalent,
// using react-native-image-picker (already a dependency) to grab the file
// and ML Kit to read it, so scanning a photo works the same as scanning live
// through the camera.
